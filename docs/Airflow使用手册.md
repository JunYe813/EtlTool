...
> ### ⚠️⚠️ `SCHEDULE` 必须是 **5 段** cron —— 写成 6 段会变成「每 2 小时」
>
> 这里踩过一个代价很大的坑：曾把快速演示写成 6 段的 `"0 0/2 * * * *"`，
> 以为第一段是「秒」、意思是「每 2 分钟」。**实际不是。**
> Airflow 底层用 croniter，6 段被当作 `分 时 日 月 周 年` 解析
> （**第 6 段是「年」，不是「秒」**），于是
> `"0 0/2 * * * *"` = 分=0、时=0/2 → **每 2 小时才跑一次**。
>
> **症状极具误导性，所以极难查：**
> - 调度器 `active`、`check_dag.py` 通过、`import_error` 是空的
> - `airflow dags list-runs` 把 schedule 原样打印成 `0 0/2 * * * *`，看不出错
> - 只是两三个小时才出一个 run —— 看起来像「调度器不推进」
>
> **唯一的铁证**在元数据库里，`next_dagrun_create_after` 会落在**整点偶数小时**：
>
> ```bash
> PGPASSWORD='...' psql -h 127.0.0.1 -p 5434 -U postgres -d airflow -c \
>   "SELECT dag_id, next_dagrun, next_dagrun_create_after FROM dag;"
> # next_dagrun_create_after = 2026-09-15 10:00:00+00   ← 整点偶数小时 = 段数写错了
> ```
>
> `python scripts/demo_replay.py preview` 现在会把它算成 **7200 秒**并直接报 `[FAIL]`。
> （早期版本误以为 6 段是「秒 分 时 …」，对同一个错误表达式给出假的 `[OK]` ——
> **诊断工具和配置犯了同一个误解，就互相放过了**，这才是它拖了几个小时的原因。）
