...
> - 待办与历史决策 → `docs/项目改进清单.md`
>
> ⚠️ 注意：根目录的 `项目大纲.md` 是**实施前的规划稿**，里面的 ODS 表名（`ods_customer` 等）
> 和实际实现（`olist_customers_dataset` 等）不一致，**已不适用**，只作历史记录。
...
| `pg_sql.sql` | 排查/对账草稿本 | 与本项目无关，不算在建仓脚本里 |
...
| `支付表对账总结.md` | 支付对账的分析结论（gitignore，仅本地） |
...
梳理过程中发现的**两处文件清单不一致**，都属于「同一个清单在多个地方各写一份」：

### 8.1 `scripts/run_sql.py` 的 `STEPS` 漏了两张表 ⚠️

```python
STEPS = {
    "dwd": ["sql/01_dwd.sql"],          # ✗ 缺 sql/04_dwd_payment.sql
    "ads": [ ... 7 个文件 ... ],         # ✗ 缺 sql/05_ads_payment_reconcile.sql
}
```

| 步骤 | 应有 | 实际 | 缺 |
|---|---|---|---|
| `--steps dwd` | `01_dwd.sql` + `04_dwd_payment.sql` | 只有 `01_dwd.sql` | **`sql/04_dwd_payment.sql`** |
| `--steps ads` | 8 个（4 增量 + 3 全量 + 视图） | 7 个 | **`sql/05_ads_payment_reconcile.sql`** |

**后果**：用 `python scripts/run_sql.py --steps dwd` 手工建仓会**静默少建两张表** ——
不报错、不提示，只是后面的增量 SQL 找不到表，或者看板的支付对账页空白。

**不影响调度路径**：调度读的是 `run_all.INCREMENTAL_FILES_BY_LAYER` 和 `ads_build.ADS_FILES`，
那两处是完整的。所以这个坑只在「手工按步骤名建仓」时触发。

**修法**（与第六节「单一来源」原则一致）：`STEPS` 不要再手抄，改成引用唯一定义处 ——

```python
from run_all import LAYERS          # DWD / DWS 全量建表清单
from ads_build import ADS_FILES     # ADS 完整清单（增量组 + 全量组 + 视图）
...
### 8.2 `项目大纲.md` 是实施前的规划稿

里面的 ODS 表名（`ods_customer` / `ods_order` / `ods_order_item`）与**实际实现**
（`olist_customers_dataset` / `olist_orders_dataset` / `olist_order_items_dataset`）完全不同。

**只作历史记录，阅读时别当现状。** 要看现状请以 `docs/数据字典.md` 和 `sql/00_ods.sql` 为准。
